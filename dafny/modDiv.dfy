 ...
 * assert remainder <= a;
 *
 * Using the definition of isModDiv and not the method implementation, we are creating our
 * lemmas.
 *
...
module ModDiv {
     ...
     * assert remainder <= a;
     */
    function isModDiv(a: nat, b: nat, division: nat, remainder: nat): bool
        requires b > 0;
        requires a >= 0;
    {
        var valid := if ! ( division * b + remainder == a ) then false
        else if ! ( remainder < b ) then false
        else if ! ( remainder <= a ) then false
        else true;

        assert valid ==> division * b + remainder == a;
        assert valid ==> a - remainder == division * b;
        valid
    }
    ...
    method mod(a: nat, b: nat) returns (remainder: nat)
       requires b > 0;
       requires a >= 0;
    {
        var division;
        division, remainder := modDiv(a, b);
        assert b * division + remainder == a;
    }

    method div(a: nat, b: nat) returns (division: nat)
       requires b > 0;
       requires a >= 0;
    {
        var remainder;
       ...
       ensures rModSum == rSum;
    {
        ...
        if ( rModSum >= rSum ) {
            ...
            assert divFactor > 1 ==> div * divFactor >= div;
            assert divFactor == 0;
            assert rModSum - rSum == div * divFactor;
        ...
        // assert r1 + r2 < 2 * div;
        // assert 2 * div > divModSum * div + rModSum;
        // assert 2 * div - divModSum * div > rModSum;
        // assert div * ( 2 - divModSum ) > rModSum;
    }
}