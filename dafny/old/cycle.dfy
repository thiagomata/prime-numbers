...
module Cycle {
    ...
    lemma checkCyclePos(list: seq<nat>, pos: nat, key: nat, result: nat)
        requires |list| > 0;
        requires key == ModDiv.mod(pos,|list|);
    ...
    {
        assert forall k : nat :: |list| <= k < |listCycle| ==> ModDiv.mod(k,|list|) == ModDiv.mod(k - |list|,|list|);
        assert forall k : nat :: 0 <= k < |list| ==> ModDiv.mod(k,|list|) == k;
        forall k : nat :: 0 <= k < |listCycle| ==> listCycle[k] == list[ModDiv.mod(k,|list|)]
    }
