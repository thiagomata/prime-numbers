...
## `gap-heatmap-2focused-aligned.svg`

Uses the same 2-focused compression and colors, but shifts each row by the
compressed-index difference of 2-gaps shared with the previous row and wholly
below that previous head squared. The observed shift is zero while advancing
the head only shortens a leading non-2 run, and one when a complete compressed
unit disappears. The straight lines show the checked alignment within the
observed safe prefix; they neither prove a theorem nor establish lineage beyond
that prefix.

![gap-heatmap-2focused-aligned](gap-heatmap-2focused-aligned.svg)
