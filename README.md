...
The article [Formal Verification of Cyclic Lists](./articles/chapter4/cycle.md)
...
The article [Structural Properties and Signed Boundaries of 2-Gaps in Sieve Sequences](./articles/chapter6/gap-dynamics.md)
...
The article [Survival Frontiers in Balanced 2-Gap Companion Processes](./articles/chapter7/survival-frontiers.md)
studies companion processes that reproduce the exact global growth of 2-gaps
from Gap Dynamics but vary where each filter removes them, comparing a
filter's local destruction rate against the random baseline $2/r$.

```math
\begin{aligned}
N_{k+1} &= (r_k - 2) N_k
\quad &\text{[Global Recurrence, Allocation-Independent]} \\
w_r = 1 + c\log r,\ c < 1
&\implies \text{eventual square-window occupancy}
\quad &\text{[Square-Window Frontier]} \\
w_r = 1 + c\log r,\ c < \tfrac12
&\implies \text{infinitely recurring head 2-gaps}
\quad &\text{[Head-Recurrence Frontier]} \\
\end{aligned}
```

These frontiers are conditional on the stated spatial, availability, and
mixing premises; none of them is proved for the real sieve, and Stainless
