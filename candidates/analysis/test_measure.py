...
Exits 0 if every assertion holds, 1 otherwise. This is the empirical analog of
`green-to-green`: it must be green before adding any measurement column and
re-run (still green) after. No pytest dependency -- stdlib assert + exit code,
in the style of the presentation repo's verify.py.
...
def test_hand_check_q5_p3():
    ...
    check("surviving==3", res["surviving"] == 3, str(res))
    check("G_local==9 (all odd pairs are 2-gaps)", res["G_local"] == 9, str(res))
    check("destroyed==6 (each mult of 3 kills two gaps)", res["destroyed"] == 6, str(res))
    check("A_worst==3", res["A_worst"] == 3, str(res))
    check("surplus==6 (G_local-A)", res["surplus"] == 6, str(res))
    check("destroyed==2*A_worst (p=3 double-count)", res["destroyed"] == 2 * res["A_worst"], str(res))
    check("destroyed+surviving==G_local",
          res["destroyed"] + res["surviving"] == res["G_local"], str(res))
...
def test_hand_check_q7_p5():
    ...
    check("G_local==6", res["G_local"] == 6, str(res))
    check("surviving==4", res["surviving"] == 4, str(res))
    check("destroyed==2", res["destroyed"] == 2, str(res))
    check("A_worst==2", res["A_worst"] == 2, str(res))
    check("surplus==4", res["surplus"] == 4, str(res))
    # The clean-transition bound: destroyed <= A_worst holds for p >= 5.
    check("destroyed<=A_worst (p>=5 isolation)", res["destroyed"] <= res["A_worst"], str(res))
    check("destroyed+surviving==G_local",
          res["destroyed"] + res["surviving"] == res["G_local"], str(res))
...
def test_structural_identities():
    ...
    while transitions < 60:  # p up to a few hundred
        ...
        check(f"window_len==q^2-q @p={p}", res["window_len"] == q * q - q, str(res))
        # (b) surviving never negative
        check(f"surviving>=0 @p={p}", res["surviving"] >= 0, str(res))
        # (c) destroyed in [0, G_local]
        check(f"0<=destroyed<=G_local @p={p}",
              0 <= res["destroyed"] <= res["G_local"], str(res))
...
def test_surviving_means_twin_prime():
    ...
    while transitions < 40:
        ...
        check(f"surviving>0 == has_twin @p={p},q={q}",
              (res["surviving"] > 0) == has_twin,
              f"surviving={res['surviving']} has_twin={has_twin}")
        transitions += 1
        p = q
        q = int(nextprime(p))
...
# ---------------------------------------------------------------------------
# 4. Cross-check G_local convention against results.csv where it overlaps
# ---------------------------------------------------------------------------
# results.csv (from the @extern Scala EmpiricalRunner) uses window [p, p^2),
# NOT [q, q^2). So we cannot compare G_local directly. What we CAN check is
# internal consistency: our own G_local for window [q, q^2) recomputed two ways
# (sieving vs. survivor-list diff) agrees. This guards against a sieve bug that
# a convention-offset cross-check would hide.
...
def test_g_local_self_consistent():
    ...
    for _ in range(30):
        ...
        check(f"G_local matches independent sieve @p={p}",
              res["G_local"] == g_local_independent,
              f"{res['G_local']} vs {g_local_independent}")
        p = q
        q = int(nextprime(p))
...
def test_candidate_columns_q7_p5():
    ...
    res = lib.transition(p=5, q=7, primes_below_p=primes_below(5))
    check("max_cluster_in_width_p==1", res["max_cluster_in_width_p"] == 1, str(res))
    check("max_cons_destroyed_run==1", res["max_cons_destroyed_run"] == 1, str(res))
    check("d_head==4", res["d_head"] == 4, str(res))
    ...
    check("destruction_rate==1/3",
          abs(res["destruction_rate"] - (1.0 / 3.0)) < 1e-12, str(res))
    check("gap_2_over_p==0.4", abs(res["gap_2_over_p"] - 0.4) < 1e-12, str(res))
    # main_term > 0 and E_q is a finite number
    check("main_term>0", res["main_term"] > 0, str(res))
    check("E_q finite", res["E_q"] == res["E_q"], str(res))  # not NaN
    # residue_max_dev and endpoint_bias are finite and >= 0
    check("residue_max_dev>=0", res["residue_max_dev"] >= 0, str(res))
    check("endpoint_bias>=0", res["endpoint_bias"] >= 0, str(res))
...
def test_cross_check_gaps_csv():
    ...
    for head in (7, 13, 31, 53, 101, 223, 521, 887):
        ...
        if gsv and gsv[-1] >= hi - 1:
            check(f"survivors==gaps.csv over full window @head={head}",
                  mine_win == gsv_win,
                  f"mine={len(mine_win)} gaps={len(gsv_win)}")
        else:
            check(f"survivors prefix==gaps.csv (partial) @head={head}",
                  gsv_win == mine_win[: len(gsv_win)],
                  f"mine={len(mine_win)} gaps={len(gsv_win)}")
...
def test_hand_check_q11_p7():
    ...
    check("G_local==11", res["G_local"] == 11, str(res))
    check("surviving==8", res["surviving"] == 8, str(res))
    check("destroyed==3", res["destroyed"] == 3, str(res))
    check("A_worst==4", res["A_worst"] == 4, str(res))
    check("max_cluster_in_width_p==2", res["max_cluster_in_width_p"] == 2, str(res))
    check("max_cons_destroyed_run==2", res["max_cons_destroyed_run"] == 2, str(res))
    check("d_head==0", res["d_head"] == 0, str(res))
    check("destroyed+surviving==G_local",
          res["destroyed"] + res["surviving"] == res["G_local"], str(res))
...
def test_candidate_columns_exact_q7_p5():
    ...
    res = lib.transition(p=5, q=7, primes_below_p=primes_below(5))
    check("residue_max_dev==0.8 (#12)",
          abs(res["residue_max_dev"] - 0.8) < 1e-12, str(res))
    check("endpoint_bias==1/7 (#13)",
          abs(res["endpoint_bias"] - (1.0 / 7.0)) < 1e-12, str(res))


def main():
    print("candidate stress-test: green gate")
    print(f"numpy={__import__('numpy').__version__} sympy={sympy.__version__}")
    print()
    test_hand_check_q5_p3()
    test_hand_check_q7_p5()
    test_structural_identities()
    test_surviving_means_twin_prime()
    test_g_local_self_consistent()
    test_candidate_columns_q7_p5()
    test_hand_check_q11_p7()
    test_candidate_columns_exact_q7_p5()
    test_cross_check_gaps_csv()
    print()
    if FAILURES:
