...
| **DEFERRED (UNMEASURED)** | whole-period or expanded-zone quantity; not touched by the window or lineage passes | #5, #6, #7, #9, #16, #26 |
| **ALGEBRA-FIRST** | derived from proved algebraic bounds after the empirical program; bounded falsifier checks are recorded where available, but the next action is proof rather than broader measurement | #19, #20, #21, #22, #23, #24, #27, #28 |
...
26. [Sub-CRT strike decoherence](sub-crt-strike-decoherence.md) — **[deferred; pairwise cohort content proved exhausted]** the two-body, frequency-resolved analogue of #10 on the mixing side: cross-head agreement matching the reference product form, and local strike-placement spectra lying in the position-blind permutation band above a decoherence band. Complete-period orthogonality is proved but cannot localize (the `LR` Bessel obstruction), and fixed-cohort pairwise covariances are exactly `-K_rK_r'/N` by destruction disjointness, so the open content is single-layer placement shape, higher-order joints, and cross-head pairs. The position-blind index spectrum property supplies the null; measurements E1–E4 unrun.
27. [Window innovation orthogonality](window-innovation-orthogonality.md) — **[algebra-first; global layer identity proved, local form open]** the strongest natural local form of "the new layer has low correlation with all previous ones": window correlations of each layer's centered strikes against the full past span (products included) stay at the position-blind fluctuation scale, equivalently the window-restricted innovation Gram matrix is near-typical. The global innovation identity is proved (conditional mean zero, span orthogonality, adaptive Pythagoras), making every window cross term an instance of the hypothesis and yielding local approximate Pythagoras — the signed mean-square cancellation the terminal chain lacks, in the basis where the complete-period answer is exact. Next: exact restricted-Gram computation (N3) and the span-regression falsifier (N1).
28. [Dream sequence self-propagating invariant](dream-sequence-self-propagating-invariant.md) — **[algebra-first; perpetuity theorem and global component proved, spacing lemma open, nonemptiness not claimed]** the inverted target: a property set `P` that reproduces itself under the sieve transition and forces a safe-window 2-gap, so one seed stage yields certificates at every later head. Component 1 proved today: the global 2-gap density relative to the Mertens benchmark is an exactly conserved quantity of the dynamics (every placement, every layer). Component 2 (Lemma A, open): local spacing self-preservation at the Mertens scale — naive degradation diverges; the rigid kill geometry (`h=0,+-2 mod r` elevated survival) is the friendly arithmetic. Mirror of the proved nightmare invariant (absence stability). Next: Lemma A.
