package v1.chapter6.seq.properties
...
object SeqProperties {
  def firstValuesMatchPrev(seq: Seq, pos: BigInt): Boolean = {
    require(pos >= 0)
    require(seq.previous.size > pos)
    seq.previous(pos) == seq(pos)
  }.holds

  def firstValuesMatchFirstPosInLoop(seq: Seq, pos: BigInt): Boolean = {
  ...
  def nextValuesMatchLoop(seq: Seq, pos: BigInt): Boolean = {
    require(pos >= seq.previous.size)
  ...
  def prevValuesMatchLoop(seq: Seq, pos: BigInt): Boolean = {
    require(pos >= seq.previous.size)
    require(pos > 0)
   ...
   */
  def seqPosMatchSeqLoop(from: BigInt, to: BigInt, seq: Seq): Boolean = {
    ...
    def compareTwoSeqPos(from: BigInt, to: BigInt, seq: Seq): BigInt = {
    require(from >= seq.previous.size)
    require(to >= from)
    decreases(to)
    if (from == to) {
  ...
  def compareTwoSeqLoop(from: BigInt, to: BigInt, seq: Seq): BigInt = {
    require(from >= seq.previous.size)
    require(to >= from)
    decreases(to)
    if (from == to) {
