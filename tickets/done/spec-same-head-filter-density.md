...
- `tickets/active/spec-same-head-filter-density-proof-review.md` — review note for the verified same-head size proof, with human and mathematical statements for each load-bearing lemma.
...
8. Current code state has moved past the original B2 wrapper surface. The next
   real micro-goal is the row-major/column-major transpose:

```text
countGeneratedHeadMultiplesRange(0, period * head)
==
countGeneratedHeadMultiplesByStrideOffsets(0, period)
```

   A direct wrapper timed out at this equality. Do not retry that shape. Use a
   same-recursion transpose proof, or move the final theorem into
   `SpecDerivedSieveSequence` only if the missing bridge is explicitly replaced
   by a verified `period == nextResidues(cycle).size` / equivalent alignment
   theorem.
...
| 2026-07-14 | Verified the generated-prefix removed-count bridge. `SpecSieveSequence.assertGeneratedHeadMultiplePrefixCount(k)` proves the concrete interval count `countAcceptedHeadMultiplesBetween(head, apply(k))` is exactly the generated-prefix count of old emitted values divisible by `head`; supporting helpers prove additivity and zero contribution from skipped intervals. Full `just verify` passed (`13757 valid, 0 invalid, 0 unknown`). | The remaining theorem has been reduced without narration to `countGeneratedHeadMultiplesPrefix(period * head.value) == period`. The current Track B2 wrappers do not prove this concrete recursive count; they expose only algebraic density equalities. Next work must either build a real predicate-count density theorem over `AllPrimesSoFarList`/`SpecSieveSequence`, or explicitly document Track B2 as blocked before switching to the existing B1 coprime-step/residue machinery. |
| 2026-07-14 | Verified the spec-local one-per-stride counting atoms. `countGeneratedHeadMultiplesRange` and `assertGeneratedHeadMultiplesPrefixMatchesRange` connect the prefix counter to an index range. `countGeneratedHeadMultiplesStrideFrom` plus `assertGeneratedHeadMultiplesStrideMatchesZeroOffsets` connects each old-period offset column to `SieveUtils.countZeroOffsets`. `assertGeneratedHeadMultiplesStrideOne` proves each column removes exactly one value when `mod(tailPrimorial, head) != 0`; `assertGeneratedHeadMultiplesByStrideOffsetsCount` sums columns to `period`. Full `just verify` passed (`13923 valid, 0 invalid, 0 unknown`). | The attempted direct transpose `countGeneratedHeadMultiplesRange(0, period * head) == countGeneratedHeadMultiplesByStrideOffsets(0, period)` timed out at the equality postcondition (`15 valid, 1 unknown`, 300s). The code now keeps `assertGeneratedHeadMultiplesRangeMatchesStrideOffsets` as an explicit conditional boundary rather than a fake proof. Next work must prove this transpose by same-shape recursion, or switch to `SpecDerivedSieveSequence` only with a verified alignment theorem replacing the transpose. |
| 2026-07-14 | Verified the same-shape transpose route. The proof introduces bounded-height stride/column counters, range front/append lemmas, column-step and zero-height lemmas, then proves `assertGeneratedHeadMultiplesRangeMatchesStrideOffsetsUntil(limit, period)` by induction on `limit`. The old conditional boundary was refactored into a real proof, and `assertGeneratedHeadMultiplesPrefixExpandedCount(period)` proves `countGeneratedHeadMultiplesPrefix(period * head) == period`. | The final same-head size theorem is now verified as `SpecSieveSequence.assertSameHeadExtendedFilterCount(period)`: assuming `apply(period) == head + M` and `mod(M, head) != 0`, the accepted non-head-multiple count over `[head, head + head*M)` is `period * (head - 1)`. Next work, if desired, should connect this spec-local theorem to the real `next`/cycle size object without reintroducing sorting/rotation or residual arguments before their alignment lemmas are explicit. |
| 2026-07-14 | Added `tickets/active/spec-same-head-filter-density-proof-review.md` for team review. It restates each load-bearing lemma in human and mathematical terms, explains why the lemma exists, and shows how the lemmas compose into the same-head size theorem. | Use the review note as the entry point for auditing the verified size proof. It does not claim the real `seq.next`/cycle-size bridge is finished. |
