# Audit One-Layer Versus Global Collision-Budget Claims

**Created:** 2026-07-29
**Updated:** 2026-07-29
**Status:** Complete
**Depends on:**
`prove-endpoint-observable-sampling-2026-07-28.md`,
...
Do not reprove unrelated properties. Inventory and classify every hit before
editing it.
...
- `verify-19-21-escape-wall-2026-07-27.md` — umbrella classification of
  candidates #19--#23. It now records local-ellipse composition as a failed
  path.
- `prove-accepted-strike-mean-square-2026-07-27.md` — candidate #23's
  completed generic algebraic audit. Separate #13/#23 representation work
  must not be mistaken for the direct weighted scalar estimate.
- `algebraic-conditioned-survival-2026-07-27.md` — origin of candidate #21's
  weighted global allowance and relevant terminology.
...
3. the still-open direct aggregate theorem for `sum_i w_i Q_i`.

The audit is complete when all relevant files are classified, every confirmed
stale or ambiguous claim is corrected, unaffected claims are recorded as such,
and catalogs/tickets point to the corrected boundary.

## Strategy

1. Build a complete textual inventory using the terms `one-layer`,
   `collision budget`, `harmful scalar`, `ellipse`, `rho_*`, properties
   #60--#65, and candidates #12/#13/#14/#19/#21/#23.
2. Read each hit in context and classify it:
   - **Correct:** explicitly one-layer or explicitly aggregate.
   - **Ambiguous:** mathematically valid locally but worded as if cumulative.
   - **Incorrect:** asserts or relies on local-to-global composition.
   - **Unaffected:** discusses another budget or an exact identity.
3. Correct one file at a time, with a diff check after each change.
4. Re-run the inventory and link checks.
...
- Property catalog entries #57, #59, #60, #63, and #64 now explicitly use
  one-layer/local scope.
- The user resumed the audit again after the second required procedural stop.
- Candidate catalog #22 now pairs harmless dispersion with restricted #12's
  direct weighted harmful norm; #23 is labeled a fallback scalar component.
- Candidate #23's opening and Limitation now describe its separate #13+#23
  estimate as a valid fallback and restricted #12 as the preferred interface.
...
- Final stale-role and local/global searches are clean.
...
- `git diff --check` passes; Markdown-only corrections require no Stainless
  verification.
...
- **Assumption:** uses of “collision budget” may refer either to one layer or
  ...
  **Validation:** inspect local definitions of `T`, `W`, `w_i`, and sums.
- **Risk:** historical learning-log entries accurately describe what was
  believed at the time but look current.
  **Validation:** change current-state conclusions; retain historical rows
  unless they make an unqualified mathematical claim that is now false.
- **Risk:** article edits trigger the three-representations and framing rules.
  **Validation:** if an article contains a confirmed error, inspect the whole
  containing property section and its abstract/introduction/conclusion before
  editing.
...
- **Treating every mention of candidate #21 as stale:** incorrect because
  many files state exact decompositions or explicitly one-layer comparisons.
  Retry only after reading the local definition of the allowance.
...
Done. Future scalar work must treat restricted #12, or the fallback #13+#23
representation, as a terminal theorem at candidate #21's global allowance.
Candidate #22 remains an independent distribution diagnostic. Properties
#60--#64 remain one-layer inputs only.

## Validation

- Confirm all modified links resolve.
- Run focused stale-claim searches after every correction group.
- Run `git diff --check` after every file and once repository-wide.
- No Scala changes are planned. If any becomes necessary, stop and begin the
  green-to-green verification protocol.
- Preserve the unrelated staged giant CSV.
