...
| 2026-07-23 | User: make the next steps clear in the README. The candidates/README.md had the corrected index but NO next-steps section -- a reader had no entry point for what to do after the window pass. | Added a "Next Steps" section to candidates/README.md with three ordered priorities: (1) the fixed-future-window multi-layer lineage experiment as highest priority -- it retrofits the correct tests for #4 (cyclic run), #10 (post-filter E_q), #12 (nu E margin), #13 (one-sided H(2L/N+b+)<L margin) AND tests #14's real interval/partial-sum + hereditary premise; (2) whole-period candidates #5/#6/#7/#9 via targeted partial-period extreme-value measurement; (3) the formal-verification gap as a separate effort. Plus a strategic-framing subsection: #2 is the terminal target, #4/#14 the leading mechanisms, #12/#13 only in conditioned one-sided-margin form, #1/#8 terminal checks not mechanisms, #5-#9 lower priority unless window mechanisms fail. |
