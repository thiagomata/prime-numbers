...
| 2026-06-20 | The period wrapper now verifies as `assertSkipUntilNonMultiple(nextSeq, k, period)`. The proof uses `bound = k + p * period`, block shifting, and the reverse acceptance bridge to show the endpoint passes the new front filter, then delegates to `assertNextSuccessorIsFirstSurvivor`. Verification is green at 6943 valid, 0 invalid, 0 unknown. | This completes the period-based skip-until-non-multiple property for the immediate-skipped-successor case. The remaining gap-merge work can now build corollaries over sums of old gaps rather than re-proving the first-survivor equality. |
