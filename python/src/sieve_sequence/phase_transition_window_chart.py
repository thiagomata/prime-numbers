...
exactly on the article's own square-window boundary: it declines to zero
only logarithmically, since lambda(Q) is proportional to 1/(log Q)^2. It is
the failure-side threshold this chart is about, so it gets the solid stroke;
every other line is dashed.
...
def draw(rows):
    ...
    for i, line in enumerate([
        ...
        "",
        "c=1 (w_r=1+log r) is the exact boundary:",
        "it declines only as 1/(log Q)^2,",
        "so slowly, but still tends to zero",
        "on the failure side.",
    ]):
