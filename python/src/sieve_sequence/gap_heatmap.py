...
def shared_two_gap_offsets(stages):
    """Return cumulative display offsets that align the shared safe 2-gaps.

    For each adjacent pair of stages, every 2-gap wholly below the previous
    head squared is unchanged by that sieve step.  We locate those 2-gaps by
    their raw starting values, compare their indices after 2-focused
    compression, and require them all to imply the same zero- or one-cell
    shift.  A data set that cannot support that exact safe-prefix alignment is
    rejected instead of being drawn with a misleading coordinate system.
    """
    if not stages:
        return []

    def two_positions(stage):
        positions = {}
        compressed_index = 0
        in_non_two_run = False
        left = stage["head"]
        for gap in stage["gaps"]:
            if gap == 2:
                if in_non_two_run:
                    compressed_index += 1
                    in_non_two_run = False
                positions[left] = compressed_index
                compressed_index += 1
            else:
                in_non_two_run = True
            left += gap
    ...
    positions_by_stage = [two_positions(stage) for stage in stages]
    offsets = [0]
    for row_index in range(1, len(stages)):
        previous_stage = stages[row_index - 1]
        previous_positions = positions_by_stage[row_index - 1]
        current_positions = positions_by_stage[row_index]
        safe_shared_starts = sorted(
            start
            for start in previous_positions.keys() & current_positions.keys()
            if start + 2 < previous_stage["head"] ** 2
        )
        if not safe_shared_starts:
            raise ValueError(
                f"stages {row_index - 1} and {row_index} have no shared safe 2-gap"
            )

        anchor = safe_shared_starts[0]
        shift = previous_positions[anchor] - current_positions[anchor]
        if shift not in (0, 1):
            raise ValueError(
                f"stages {row_index - 1} and {row_index} imply unsupported shift {shift}"
            )
        if any(
            previous_positions[start] - current_positions[start] != shift
            for start in safe_shared_starts
        ):
            raise ValueError(
                f"stages {row_index - 1} and {row_index} do not share one safe-prefix shift"
            )
...
def build_compressed_grid_png(
    stages, compressed_per_row, color_by_value, width, stagger=0, row_offsets=None
):
    ...
    `row_offsets` supplies an exact offset for each row when fixed staggering
    is not the appropriate coordinate system."""
    if row_offsets is None:
        row_offsets = [stagger * row_idx for row_idx in range(len(stages))]
    elif len(row_offsets) != len(stages) or any(offset < 0 for offset in row_offsets):
        raise ValueError("row_offsets must contain one non-negative offset per stage")
    full_width = width + max(row_offsets, default=0)
...
def build_compressed_heatmap(
    stages,
    stagger=0,
    png_name="gap-heatmap-2focused.png",
    title_suffix="",
    row_offsets=None,
) -> Canvas:
    ...
    the equalized sequential ramp. Optional exact row offsets align the result
    in compressed rather than raw-gap coordinates."""
...
def main() -> None:
    ...
    aligned_offsets = shared_two_gap_offsets(stages)
    compressed_aligned = build_compressed_heatmap(
        stages,
        png_name="gap-heatmap-2focused-aligned.png",
        title_suffix=" (rows aligned by shared pre-square 2-gaps)",
        row_offsets=aligned_offsets,
    )
    compressed_aligned_path = os.path.join(
        OUT_DIR, "gap-heatmap-2focused-aligned.svg"
    )
    save(compressed_aligned, compressed_aligned_path)
    print(f"wrote {compressed_aligned_path}")
