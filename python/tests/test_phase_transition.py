...
Run:  pytest python/tests/test_phase_transition.py
...
from sieve_sequence import phase_transition as lib
...
def test_fixed_w_hand_derived():
    ...
    assert close(log10_lambda, expected), f"w=1, Q=1000 matches direct computation: got {log10_lambda}, expected {expected}"
...
def test_fixed_w_eventually_recovers_for_large_w():
    ...
    assert dip < 0, f"w=10 is negative (dipped) at log10(Q)=10: got {dip}"
    assert recovered > 0, f"w=10 has recovered positive by log10(Q)=50: got {recovered}"
    assert lib.log10_window_occupancy_fixed_w(100.0, 10.0) > recovered, "w=10 keeps climbing from Q=50 to Q=100"
...
def test_fixed_w_always_eventually_diverges():
    ...
    for w in (1.0, 3.0, 6.0, 10.0, 50.0):
        ...
        assert farther > far, f"w={w} still climbing at huge log10(Q): far={far}, farther={farther}"
...
def test_log_growth_c_zero_matches_fixed_w_one():
    ...
    for log10_Q in (2.0, 3.0, 5.0, 8.0):
        ...
        assert close(a, b), f"c=0 == fixed w=1 at log10(Q)={log10_Q}: {a} vs {b}"
...
def test_log_growth_hand_derived():
    ...
    assert close(log10_lambda, expected), f"c=0.5, Q=1000 matches direct computation: got {log10_lambda}, expected {expected}"
...
def test_log_growth_threshold_at_c_equals_one():
    ...
    for c, should_increase in [(0.3, True), (0.7, True), (1.5, False)]:
        ...
        if should_increase:
            assert far > near, f"c={c} (<1) diverges: far > near: near={near}, far={far}"
        else:
            assert far < near, f"c={c} (>1) decays: far < near: near={near}, far={far}"
...
def test_constant_share_eventually_fatal():
    ...
    assert far < near, f"constant share decreasing well before log10(Q)=6: {near} -> {far}"
    assert farther < far, f"constant share keeps decreasing (not recovering): {far} -> {farther}"
...
def test_constant_share_worse_than_any_fixed_w():
    ...
    assert share < fixed, f"constant share below fixed w=50 at log10(Q)=10: share={share}, fixed_w50={fixed}"
...
def test_head_probability_hand_derived():
    ...
    assert close(p, expected), f"c=0.5, Q=1000 matches direct computation: {p} vs {expected}"
...
def test_cumulative_head_sum_diverges_below_half_converges_above():
    ...
    for c, label, ratio_check in [
        ...
        assert ratio_check(ratio), f"c={c} ({label}): ratio matches expected trend: ratio={ratio:.3f}"