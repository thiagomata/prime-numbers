...
Run:  pytest python/tests/test_lineage.py

The empirical analog of green-to-green: must be green before scaling Q.
Every cited number must come from a run that passed this suite.
...
def test_sigma_self_consistency():
    ...
    assert M == 6, f"stage{{2,3}} M==6: M={M}"
    assert g == [4, 2], f"stage{{2,3}} gaps==[4,2]: gaps={g}"
    s2, _ = lib.sigma_r_for_layer(2, 5, [2, 3])
    assert s2 == 10, f"sigma_5(2)==10 (r*min_gap=5*2): sigma_5(2)={s2}"
    # sigma_5(T=2) = r*(M - max_gap) = 5*(6-4) = 10  (sums 1 gap = min gap)
    sT2, _ = lib.sigma_r_for_layer(len(g), 5, [2, 3])
    assert sT2 == 5 * (6 - max(g)), f"sigma_5(T=2)==5*(6-max_gap=4)==10: sigma_5(T)={sT2}"

    # Stage {2,3,5}: M=30, gaps [6,4,2,4,2,4,6,2], T=8. r=7.
    g3, M3 = lib.full_period_cofactor_gaps([2, 3, 5])
    assert M3 == 30, f"stage{{2,3,5}} M==30: M={M3}"
    assert len(g3) == 8, f"stage{{2,3,5}} T==8: T={len(g3)}"
    s_min, _ = lib.sigma_r_for_layer(2, 7, [2, 3, 5])
    assert s_min == 14, f"sigma_7(2)==7*2==14: sigma_7(2)={s_min}"
    # sigma_7(T=8) = 7*(30 - max_gap=6) = 7*24 = 168
    s_full, _ = lib.sigma_r_for_layer(len(g3), 7, [2, 3, 5])
    assert s_full == 7 * (30 - max(g3)), f"sigma_7(T=8)==7*(30-6)==168: sigma_7(T)={s_full}"
...
def test_layer0_sigma_undefined():
    ...
    assert M == 2, f"stage{{2}} M==2: M={M}"
    assert len(g) == 1, f"stage{{2}} T==1: T={len(g)}"
    # sigma_r_for_layer should raise for k=2 when T=1
    try:
        lib.sigma_r_for_layer(2, 3, [2])
        assert False, "sigma_3(2) on T=1 raises: did not raise"
    except ValueError:
        pass
...
def test_layer1_hand_Q17():
    ...
    assert res["G_r_window"] == 45, f"G_r_window==45: {res}"
    assert res["destroyed"] == 18, f"destroyed==18: {res}"
    assert res["surviving"] == 27, f"surviving==27: {res}"
    assert res["destroyed"] + res["surviving"] == res["G_r_window"], f"destroyed+surviving==G_r: {res}"
    # sigma_5(2) defined here (T=2): r*min_gap = 5*2 = 10
    assert res["sigma_r_2"] == 10, f"sigma_r_2==10: {res}"
    # sigma_5(T=2) = r*(M - max_gap) = 5*(6-4) = 10 (sums 1 gap)
    assert res["sigma_r_T"] == 10, f"sigma_r_T==10 (r*(M-max_gap)): {res}"
    assert res["M_r"] == 6, f"M_r==6: {res}"
...
def test_reading_a_population_shrinks():
    ...
    for r in [3, 5, 7, 11, 13]:
        ...
        assert pre_next == post_this_filtered, f"layer r={r}: post==pre_next (Reading A): r={r} below={below}"
        assert res["G_r_window"] >= res["surviving"], f"layer r={r}: G_rWindow>=surviving: {res}"
...
def test_cyclic_and_window_runs_both_reported():
    ...
    assert isinstance(res["cyclic_run_full_period"], int), f"cyclic_run_full_period is int: {res}"
    assert isinstance(res["window_linear_run"], int), f"window_linear_run is int: {res}"
    # both non-negative, cyclic >= window-linear in general (larger domain)
    assert res["cyclic_run_full_period"] >= 0, f"cyclic>=0: {res}"
    assert res["window_linear_run"] >= 0, f"window_linear>=0: {res}"
...
def test_margins_finite():
    ...
    for k in ["c12_margin", "c13_margin", "post_E_q"]:
        v = res[k]
        assert v == v, f"{k} finite: {res}"  # not NaN
...
def test_sigma_stable_no_wall():
    ...
    for k, v in expected.items():
        assert lib.sigma_r_stable(k) == v, f"sigma_stable({k})=={v}: got {lib.sigma_r_stable(k)}"
    ...
    for k in (2, 3, 6):
        ...
        assert s == r * expected[k], f"large-M sigma_{r}({k})=={r}*{expected[k]} no wall: got {s}, M={M}"