...
| **assertNextCycleApplyMatchesSpecNext(nextPeriod, k)** | `CanonicalCycleSieve(spec.next, nextPeriod).cycle(k) == spec.next(k)` for `k >= 0` | Next-stage structural identity (approach P1). Leg 2's `assertApplyMatches` instantiated one stage later: constructs the canonical wrapper of `spec.next` and applies the current-stage lemma. Carries next-stage versions of the constructor preconditions. Verified with 9402 valid. Does NOT prove `cycle.next()` computes this — that is P2 (open). |
