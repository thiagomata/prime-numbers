...
everywhere given a non-negative start. The core block closes with its
simplest instance: the unit cycle $[1]$, whose integral enumerates the
consecutive integers $init + i + 1$ in strictly increasing order --- the
candidate stream that the sieve-sequence articles later filter.
...
\begin{equation*}
\operatorname{CycleIntegral}([1], init)_i
= init + i + 1
...
\begin{equation*}
0 \leq a < b \implies
\operatorname{CycleIntegral}([1], init)_b
> \operatorname{CycleIntegral}([1], init)_a
