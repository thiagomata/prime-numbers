...
discrepancies. The division is deliberate: the companion article fixes and
machine-checks the sieve object; this article expands the mathematical theory
of that object without claiming that every new theorem has a Scala encoding.
...
\qquad\blacksquare\ \text{[No New Occurrences; Q.E.D.]}
```
...
\qquad\blacksquare\ \text{[Copy-Or-Merge Exhaustion; Q.E.D.]}
```
...
$b_i=a_iN_i-N_{i+1}$. The accepted-boundary model isolates the *strike-density error*
...
weighted-energy budget requires a *weighted sum of their squares*:
```math
\sum_i
...
at the given shift. The open estimate is the *relative* bound
```math
...
Every property below belongs to an adjacent investigation of the wider
research program. None is used as a premise of a theorem in this article.
The proof status is quoted from each property's own record: where a proof
is conditional, the condition family named there is repeated here, and the
record itself states the full hypotheses. **Proved** means mathematically
proved in the record; Stainless verification is not claimed for any row.

Table 1 lists the properties treated only in their own property records —
that is, properties left out of this article. Table 2 lists the eighteen
properties with a direct connection to this article, and states that
connection per row.

**Table 1 — properties left out of this article.**
