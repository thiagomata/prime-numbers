...
**Email:** thiago.henrique.mata@email.com  
...
This article tests that inequality empirically across all primes up to 1000.
...
**Intuition:** The segmented sieve isolates the window $[p, p^2]$ where twin primes could exist after filtering by all smaller primes. By computing survivors directly instead of simulating the full cycle, we avoid the exponential blowup of the full MemCycle.

**Why This Matters:** This is the empirical approach to the local density question: can we verify that $G_{\text{local}} > p$ for all sufficiently large $p$? If so, the sieve lacks the capacity to destroy all 2-gaps.
...
This implementation is verified in the [
...
**Intuition:** A 2-gap is a pair of consecutive survivors $(r, r+2)$. This is the exact structure that corresponds to twin prime candidates after filtering.

**Why This Matters:** The capacity argument hinges on comparing the number of 2-gaps ($G_{\text{local}}$) against the number of filter bullets ($p-1$). If $G_{\text{local}} > p-1$, at least one 2-gap survives each filter application — the sieve cannot extinguish all twin prime candidates.

```scala
def countTwoGaps(survivors: List[BigInt]): BigInt
```

This function is verified in the [
...
The $G/p$ ratio increases monotonically (with one small fluctuation, see §3.3) from 1.14 at the permanent crossover to 8.09 at the maximum tested $p$. The growth shows no signs of saturation.
...
### 5.1 Confirmed: The Capacity Argument Holds Empirically

| Hypothesis | Status | Detail |
|------------|--------|--------|
| **Crossing**: $\exists p_c$ s.t. $\forall p \ge p_c$, $G_{\text{local}} > p$ | [Confirmed] | $p_c = 37$; holds for all 154 subsequent primes |
| **Monotonicity**: $\delta$ never permanently dips back after crossover | [Confirmed] | Single minor fluctuation at $p=73$ (51→50); no extinction reversion |
| **Domination**: $\delta$ grows faster-than-linear with $p$ | [Confirmed] | $\delta/p$ increases from 0.14 to 7.09 across range |
...
However, **this is empirical evidence, not a formal proof**. The local density question ($G_{\text{local}} > p$) remains open in the formal verification sense. See learnings-capacity-argument.md Section 10 and 16 for the formal boundary of what is and isn't proven. This data complements the formal framework by providing empirical confirmation that the safe zone is not trivially empty.
...
| Empirical result ≠ proof | Does not replace a formal invariant | Use to guide invariant discovery; reduces search space |
...
The complete dataset is at `data/empirical/results.csv` with columns:
`k, p, p_next, G_local, delta, extinction`

The empirical runner is at `src/main/scala/v1/seq/sieve/empirical/`:
- `EmpiricalRunner.scala` — main entry point
- `SegmentedSieve.scala` — survivor computation
...
1. Mata, T. H. (2026). Generalized Gap Dynamics and Candidate Persistence via Algebraic Uniformity in Sieve Sequences. `articles/draft-generalized-gap-dynamic.md`
2. Mata, T. H. (2026). Learnings: Capacity Argument for Twin Prime Persistence. `articles/learnings-capacity-argument.md`
